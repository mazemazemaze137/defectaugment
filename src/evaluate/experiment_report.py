...
def build_classifier_report(base_dir, augmented_dir, augmented_name="原始数据 + GAN 生成数据"):
    ...
    lines = [
        ...
        f"- 训练轮数：{base['epochs']}",
        f"- 实际完成轮数：{base.get('completed_epochs', base['epochs'])}",
        f"- Batch Size：{base['batch_size']}",
        f"- 输入尺寸：{base['image_size']}x{base['image_size']}",
        f"- 学习率：{base['lr']}",
        f"- 随机种子：{base.get('seed', '')}",
        "",
        "## 样本规模",
        "",
        "| 实验组 | 原始训练样本 | 生成/增强样本 | 训练总数 | 验证样本 |",
        "| --- | ---: | ---: | ---: | ---: |",
        (
        ...
        (
            f"| {augmented_name} | {augmented['counts']['train_original']} | "
        ...
        "## 分类效果",
        "",
        "| 实验组 | 最佳 Epoch | 最佳验证准确率 | 最终验证准确率 | 训练耗时 |",
        "| --- | ---: | ---: | ---: | ---: |",
        (
            f"| 原始数据基线 | {base.get('best_epoch', '')} | {_percent(base['best_val_accuracy'])} | "
            f"{_percent(base['final_val_accuracy'])} | {base['elapsed_seconds']:.2f}s |"
        ),
        (
            f"| {augmented_name} | {augmented.get('best_epoch', '')} | {_percent(augmented['best_val_accuracy'])} | "
            f"{_percent(augmented['final_val_accuracy'])} | {augmented['elapsed_seconds']:.2f}s |"
        ),
        "",
        "## 对比结论",
        "",
        f"- 本次实验额外加入 {extra_samples} 张增强样本，训练集规模增加 {total_delta} 张。",
        (
            f"- 最终验证准确率变化为 {final_delta * 100:+.2f} 个百分点；"
            f"最佳验证准确率变化为 {best_delta * 100:+.2f} 个百分点。"
        ),
        f"- 增强实验训练耗时变化为 {time_delta:+.2f}s，主要受训练样本数量和早停轮数影响。",
        "- 建议结合多随机种子、不同增强比例和质量筛选结果解释该实验，不宜只凭单次结果下结论。",
        "",
        "## 结果文件",
        "",
        f"- 原始基线目录：`{Path(base_dir)}`",
        f"- 对比实验目录：`{Path(augmented_dir)}`",
        f"- 原始基线混淆矩阵：`{Path(base_dir) / 'confusion_matrix.png'}`",
        f"- 对比实验混淆矩阵：`{Path(augmented_dir) / 'confusion_matrix.png'}`",
        "",
    ]
    return "\n".join(lines)
...
def parse_args():
    ...
    parser.add_argument("--augmented-name", default="原始数据 + GAN 生成数据")
    return parser.parse_args()


def main():
    args = parse_args()
    report = build_classifier_report(args.base_dir, args.augmented_dir, augmented_name=args.augmented_name)
